...
**Frozen checklist (HARD):** если в parent prompt есть `## Frozen QA checklist` с `checklist_sha256` — это единственный SoT матрицы. Pack/проверка **строго 1:1** с списками `### AC+` / `### AC−` / `### §0.11` / `### Prior blockers`. FORBIDDEN: добавить пункты, переформулировать, усилить wording, или FAIL по критерию вне freeze.

**Eligible blocker (может дать FAIL → BUGFIX):**
1. Suite claim vs evidence mismatch / `suite_not_full`.
2. Пункт `AC+`/`AC−` из **frozen** checklist (или packed prompt, если freeze missing) **не** выполняется по evidence `file:line` (literal gap).
3. `§0.11` = orphan external ref (missing counterpart: API/env/storage/event/DB) — не style.
4. Leftover sunset / `sot_enforce` / behavior-smoke FAIL, если parent включил это в checks.
5. `Prior blockers` item still open after BUGFIX.
...
3. «Сделай тест/проверку строже plan AC» (exact-body parity, adversarial extras, full-corpus scan) — если этого **нет** в frozen/`AC+`/`AC−` wording.
4. Повторный raise планки на re-QA: пункт, который закрыт evidence под **тем же** frozen checklist, нельзя провалить новой более строгой интерпретацией.
5. Validate-boundary / spawn-prompt hygiene parent’а — только `BLOCKED`/`prompt_incomplete`, не product BUGFIX.
