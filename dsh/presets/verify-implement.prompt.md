...
4. Пронумеруй `AC−` → для каждого: докажи **только** по файлам из `ALLOW READ` ∩ touch-ledger (Read или `git diff -- <этот path>`). Нарушение в ALLOW∩ledger → `FAIL`. Path в ALLOW без записи в touch-ledger = чужой dirty → ignore.
