...
**Обязательные** gate’ы (когда agent enabled в scope): **explorer** (широкий codebase search; полный shard `files:` / `delta_paths_*` → SKIP) · **verify** (pre-FINISH) · **reviewer** (BACK QA).

| `subagent_type` | Phase / Когда | Обязателен? | Alias compatibility |
|-----------------|---------------|-------------|---------------------|
| `explorer` | широкий codebase search / discovery | **да**, если `MODEL_LOOP=1` **и** нет полного shard `files:` / `delta_paths_*` (иначе SKIP) | — |
...
| Режим | Поведение |
|-------|-----------|
| IMPLEMENT · REFACTOR · BUGFIX · TASK (code) | широкий поиск → **`@explorer`** (если managed). Полный shard `files:` / `delta_paths_*` → **SKIP** explorer и graphify query. Неизвестные callers → graphify затем explorer |
...
**FAIL:** code-режим сделал **широкий** codebase search без предшествующего `Agent`→`explorer` (кроме SKIP: полный `files:` / `delta_paths_*` — explorer не требовать).  
