...
- Один чат = **весь** decompose (`yaml/decompose-index.yaml` + **все** `eNN-*.yaml` из plan)
...
Источник: `memory-bank/integration/plan/<epic_id>/md/plan.md` (или `$ARGUMENTS`).
Читай `workflow-decompose.mdc` + isolation `_lean/decompose.mdc`.
Шаблоны: `.cursor/templates/decompose/index.yaml` + `.cursor/templates/decompose/epic-step.yaml`.
