...
```
backend/
├── app/                         # Доменные модули (model/schema/crud/service по фичам)
│   ├── auth/
│   ├── integrations/
│   ├── partner/
│   └── ...                      # остальные доменные области
├── api/
│   └── v1/
│       ├── api.py               # сборка v1 роутера
│       └── endpoints/           # REST endpoints (по ресурсам)
│           ├── webhook.py
│           ├── scenario.py
│           └── ...
├── core/                        # Инфраструктура/кросс-сечения
│   ├── settings.py              # конфигурация приложения
│   ├── middleware.py            # middleware
│   ├── dependencies.py          # Depends-зависимости (например service bus)
│   ├── database/                # база данных/сессии/подключения
│   │   ├── database.py
│   │   ├── session.py
│   │   └── aioodbc_session.py
│   └── ...                      # logger/exceptions/utils и т.п.
...
**Avoid:** module-level service singletons (`job_service = JobService()` in `service.py`). Use `get_*_service()` in `core/dependencies.py` or `app/<domain>/deps.py` with `Annotated[..., Depends(...)]`. See `python-anti-patterns` skill.
