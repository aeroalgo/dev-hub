...
**Source implement (если scoped):** [implement shard](../../implement/<epic_id>/…)
...
| Kind | Artifact |
|------|----------|
| Source implement | [implement shard](../../implement/<epic_id>/…) |
| BACK plan / covered_by | [plan](../../../back/plan/<epic>/md/plan.md) → `../../../back/plan/<epic>/yaml/decompose-index.yaml` |
| FRONT plan | [plan](../../../front/plan/<epic>/md/plan.md) → `../../../front/plan/<epic>/yaml/decompose-index.yaml` |
...
- Планы закрытия: canonical `back/plan/<epic>/md/plan.md`, `front/plan/<epic>/md/plan.md`, `covered_by` epic
- **Обязательно:** секция **«Работы по gap»** — не только таблица ID; для каждого G-* подробно что сделать
- Runtime contract: BACK/FRONT/INTEG read the same `activeContext.md` + decompose index source of truth; `state.json` is telemetry projection, not a durable cursor.
- Gap metadata must preserve role, `decompose`, `depends_on`, completion and checkpoint data. Dependency work is sequential in one checkout; `GAP_FANOUT` is manual-only unless the runner explicitly reports otherwise.
- Rollout evidence must cover timeout/process death, `BLOCKED`/`NEED_HUMAN`, validated `resume_from_step` and rollback without resetting to first pending.
...
→ [plan](../../../front/plan/<epic>/md/plan.md)
→ decompose: `../../../front/plan/<epic>/yaml/decompose-index.yaml` (если есть)
...
→ [plan](../../../back/plan/<epic>/md/plan.md)
→ decompose: `../../../back/plan/<epic>/yaml/decompose-index.yaml`
