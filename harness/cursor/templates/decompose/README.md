...
|----------|------|
| Machine hub | `plan/<plan_id>/yaml/decompose-index.yaml` — единственный канон status |
...
**FORBIDDEN:** flat decompose directories · `yaml/index.md` · `yaml/index.yaml` · любые markdown-индексы.

**Status:** только `mark-index-status` / `finalize-step` (пишет валидированный YAML).
**IMPLEMENT load_now:** work shard + `yaml/decompose-index.yaml`.
...
**Maximal detail:** coverage и replacement cleanup хранятся в YAML-шардах DECOMPOSE; IMPLEMENT читает только текущий shard и YAML-индекс.
