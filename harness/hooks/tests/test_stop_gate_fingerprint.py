...
def test_stop_gate_no_fingerprint(tmp_path: Path, monkeypatch):
    ...
    _write_file(
        tmp_path / "loop" / "schemas" / "phase_registry.yaml",
...
def test_finish_writes_fingerprint(tmp_path: Path):
    ...
    ac_content = (
        "## load_now\n- [s02](memory-bank/back/plan/T-TEST-001/yaml/steps/s02.yaml) — s02\n\n"
        "## Handoff BACK IMPLEMENT — in progress\n- status: in_progress\n"
    )
    _write_file(tmp_path / "memory-bank" / "activeContext.md", ac_content)
    ...
    _write_file(
        tmp_path / "memory-bank" / "back" / "plan" / "T-TEST-001" / "yaml" / "decompose-index.yaml",
        idx_content,
    )
    _write_file(
        tmp_path / "memory-bank" / "back" / "plan" / "T-TEST-001" / "yaml" / "steps" / "s02.yaml",
        "schema: epic-decompose/v1\nstep_id: s02\n",
    )
    _write_file(
        tmp_path / "memory-bank" / "back" / "implement" / "T-TEST-001" / "s02.yaml",
