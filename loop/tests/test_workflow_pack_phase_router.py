...
from loop.epic_transition import load_phase_registry, _PHASE_REGISTRY_CACHE
...
def test_load_phase_registry_missing_yaml(tmp_path: Path) -> None:
    """AC+2 / cp3: pack_id with missing phase_registry path -> ValueError (TM-001)."""
    (tmp_path / "memory-bank").mkdir(parents=True, exist_ok=True)
...
def test_registry_cache_per_pack_id(tmp_path: Path) -> None:
    """cp4: Cache is per-pack-id; two different pack_ids have independent cache entries."""
    (tmp_path / "memory-bank").mkdir(parents=True, exist_ok=True)
...
def test_normalize_software_pack() -> None:
    """AC+1 / cp1: normalize_registry_phase('BACK IMPLEMENT', software_pack) -> 'IMPLEMENT'."""
    from loop.epic_transition import normalize_registry_phase
...
def test_normalize_custom_prefix() -> None:
    """AC+2 / cp2: normalize_registry_phase('SCRIPT IMPLEMENT', video_pack) -> 'IMPLEMENT' (TM-003)."""
    from loop.epic_transition import normalize_registry_phase
    from loop.workflow.schemas import WorkflowPack
...
def test_gates_from_phase_video_pack(tmp_path: Path, monkeypatch) -> None:
...
def test_gates_from_phase_auto_resolve(tmp_path: Path, monkeypatch) -> None:
...
def test_arm_phase_dsh_video_pack(tmp_path: Path, monkeypatch) -> None:
    """AC+2 / cp3: arm_phase dsh + video pack -> dsh_preset from video phase_registry (TM-004)."""
...
def test_route_command_software() -> None:
    ...
    from loop.workflow.schemas import WorkflowPack
    from loop.workflow.command_router import route_command, CommandRoute
...
def test_session_start_pack_inject(tmp_path: Path, monkeypatch) -> None:
    """AC+1 / cp1: EPIC_LOOP=1 + valid pack -> additionalContext contains pack_id and prefixes (TM-005)."""
    from harness.hooks.epic.core import session_start_payload, save_epic_state
...
def test_session_start_pack_inject_fail_safe(tmp_path: Path, monkeypatch) -> None:
...
def test_session_start_no_loop(tmp_path: Path, monkeypatch) -> None:
