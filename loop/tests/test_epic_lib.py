...
def test_discover_epic_for_pipeline_returns_resolved_identity(monkeypatch) -> None:
    ...
    identity = {
        ...
        "role": "BACK",
        "role_dir": "back",
        "decompose": "memory-bank/back/plan/T-037-loop-gap-closure/yaml/decompose-index.yaml",
    }
...
def test_post_implement_audit_handoff_has_no_standalone_epic_done(tmp_path) -> None:
    ...
    body = lib.build_post_implement_active_context(
        ...
        role_dir="back",
        epic_id="T-003",
        tracker_rel="memory-bank/back/plan/T-003/yaml/decompose-index.yaml",
        tracker_link="back/plan/T-003/yaml/decompose-index.yaml",
        phase="AUDIT",
        qa_path=None,
        cwd=tmp_path,
    )
