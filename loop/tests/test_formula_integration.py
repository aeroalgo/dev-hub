...
def test_render_then_tree_valid(tmp_path: Path):
    ...
    written_files = render_formula("hooks-epic", "T-TEST-002-render-tree", "render-tree", project_root=tmp_path)
...
"""
    ...
    assert res.returncode == 0, f"validate-decompose-tree failed: {res.stderr} {res.stdout}"
