...
## Symptom
The implement shard for a step is marked `completed` (or all checkpoints/evidence are verified done), but `yaml/decompose-index.yaml` step status remains `in_progress` or `pending`.
...
The Tier-0 repair inspects the implement shard state, verifies that step criteria were fulfilled, and updates the validated YAML index to match the completed implement step.
...
3. Check `yaml/decompose-index.yaml` to confirm it reflects `completed` for `sNN`.
