...
EVENT_TYPE_TO_TARGET_STATE: dict[LifecycleEventType, InvocationState] = {
    ...
    LifecycleEventType.TASK_STOP: InvocationState.CANCELLED,
...
def validate_receipt_epoch(
    ...
    *,
    ...
    if isinstance(receipt, dict):
        epoch_val = receipt.get("epoch")
        if epoch_val is not None and int(epoch_val) != int(expected_epoch):
            return False
        if expected_invocation_id is not None:
            inv_id = receipt.get("invocation_id")
            if inv_id is not None and str(inv_id) != str(expected_invocation_id):
                return False
        return True
    ...
    if hasattr(receipt, "epoch"):
        ...
        if epoch_val is not None and int(epoch_val) != int(expected_epoch):
            return False
        if expected_invocation_id is not None and hasattr(receipt, "invocation_id"):
...
class LifecycleReducer:
    ...
    def apply_event(self, event: InvocationEvent) -> InvocationRecord:
        ...
        with self._lock:
            ...
            # If already at target state and not terminal transition attempt
            ...
            elif event.target_state == InvocationState.RUNNING:
                first_action = True
                if not first_action_at:
                    first_action_at = event.timestamp
    ...
    def start(self, key_or_id: InvocationKey | str, actor: str) -> InvocationRecord:
        return self.transition(key_or_id, InvocationState.RUNNING, actor, first_action_taken=True)
    ...
    def pass_invocation(
        ...
        if record is not None and receipt is not None:
            if not validate_receipt_epoch(receipt, record.key.epoch, expected_invocation_id=record.invocation_id):
                raise InvalidStateTransitionError(
    ...
    def handle_task_stop(
        ...
        *,
        ...
        target_state: InvocationState = InvocationState.CANCELLED,
    ) -> InvocationRecord:
