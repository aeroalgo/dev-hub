...
- **Runner bounds:** `EPIC_SESSION_TIMEOUT_SEC` (3600), `EPIC_SESSION_KILL_GRACE_SEC` (30), `EPIC_TRANSIENT_RETRY_MAX` (30), `EPIC_SUBAGENT_RETRY_MAX` (3 retries after the initial timed-out native child), `EPIC_DEGRADED_MAX` (3), `EPIC_SESSION_LOG_LIMIT_BYTES` (10000000), `EPIC_SESSION_LOG_KEEP` (10; prune older `session-*.log` each outer iteration), `EPIC_STATUS_HEARTBEAT_SEC` (30; empty = disabled), `EPIC_STREAM_IDLE_TIMEOUT_SEC` (300; empty = disabled; idle = no `tool_use`/`tool_result`, not stream silence), `EPIC_COLLAB_WAIT_TIMEOUT_SEC` (180; native Codex `spawn_agent` → `wait` child deadline; fail-closed), `EPIC_CHAIN_ROADMAP` (0 = stop after EPIC_DONE; 1 = arm next from roadmap Queue). Zero/unlimited mode is not supported; invalid values fail closed with `invalid_runtime_config`.
...
**Anti-carry policy:** Строго запрещён перенос незакрытых требований в соседние feature-эпики (no neighbor carry) и использование скользящих окон (sliding window).
