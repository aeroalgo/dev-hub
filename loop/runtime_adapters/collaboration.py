...
1. Ровно **один** suite по `suite_command` из classifier (значение в блоке — не хардкодь). FORBIDDEN: повторный full suite, смена flags, подмена команды, thrash. FRONT: `suite_command` = Vitest+Playwright; **запрещён** `bin/pytest` / backend pytest (`suite_wrong_stack`).
...
4. `verify-qa` PASS (включая PASS с ineligible residuals) — lifecycle атомарно вызывает `mb-finish qa` и закрывает шаг. Parent после PASS обязан остановить turn: **запрещены** ручной повторный `mb-finish`, `rg`/Read по gate и новый QA-сеанс для того же PASS. `verify-qa` FAIL/BLOCKED с eligible B* → тот же путь что п.2: qa-*.yaml fail/blocked → BUGFIX, **без** repair-loop и без повторного suite в этом run. Style/naming/comments/«строже plan» → **не** BUGFIX.
...
6. Для `gate-repair` в VERIFY передавай **точную** команду проверки текущей фазы по файлам из ALLOW WRITE (targeted pytest / vitest / CLI validate — как указал parent). Полный suite из classifier — только parent в QA; не требуй full suite внутри `gate-repair`.
