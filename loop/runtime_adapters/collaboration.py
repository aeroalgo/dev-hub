...
4a. **Dirty scope (HARD):** чужой/`git status` dirty вне touch-ledger **не** blocker и **не** повод для `git checkout --` / `git restore` / `rm`. Pack `out_of_scope_*` только для path из touch-ledger текущего шага.
