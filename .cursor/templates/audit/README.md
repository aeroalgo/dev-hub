...
`converged: true` допустим только при отсутствии actionable findings и leftover-поверхностей. При наличии хотя бы одного actionable finding значение должно быть `false`.
...
- Legacy `legacy_surfaces_remaining`, `fallback_remaining` и `purge_step_present` сохраняются для brownfield cleanup gates.
