...
→ [yaml/decompose-index.yaml](../yaml/decompose-index.yaml) · [decompose-index.yaml](../yaml/decompose-index.yaml) — status SoT = yaml; index = sole tracker (не дублировать чеклист шагов здесь).
